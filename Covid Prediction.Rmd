...
```{r Quick Error Check/Scratch}
first_165 = head(df$`Los Angeles, California`, 165)
first_165ts = ts(as.numeric(first_165))

forecast(first_165ts)

predicted_10_LA = as.data.frame(as.numeric(forecast(first_165ts)$mean))
last_10_LA = as.data.frame(as.numeric(tail(df$`Los Angeles, California`, 10)))

per_error = ((predicted_10_LA - last_10_LA)/last_10_LA)*100 #percent error
print(per_error)

plot(cbind(predicted_10_LA, last_10_LA)) +
  title("LA Last 10 Day Cases")
```
